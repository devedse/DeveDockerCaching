...
export default class ACRAuthenticationTokenProvider extends AuthenticationTokenProvider{
    ...
    public getAuthenticationToken(): RegistryAuthenticationToken
    {
        if(this.registryURL && this.endpointName) {      
            let retval = new RegistryAuthenticationToken(tl.getEndpointAuthorizationParameter(this.endpointName, 'serviceprincipalid', true)!, tl.getEndpointAuthorizationParameter(this.endpointName, 'serviceprincipalkey', true)!, this.registryURL, "ServicePrincipal@AzureRM", this.getXMetaSourceClient());
            return retval;
        }
